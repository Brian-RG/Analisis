	...
	public void loadImage() {
		...
		try {
			...
			if(r==JFileChooser.APPROVE_OPTION){
				...
				int p=Integer.parseInt(JOptionPane.showInputDialog("Insert width:"));
				int l=Integer.parseInt(JOptionPane.showInputDialog("Insert Height:"));
				Loaded=new ImageIcon(path).getImage();
				xim=p;
				yim=l;
				Loaded = Loaded.getScaledInstance(xim, yim, Image.SCALE_DEFAULT);
			}
			//path+="\\src\\";
...
//			g.drawImage(Loaded,0,0,this);
			g.drawImage(Loaded, 0, 0, Loaded.getWidth(null), Loaded.getHeight(null), this);
		}
		...
		if(drawing) {
			if(bh.getSelected()==1) {
				...
				Color cur=bh.getColor();
				Graphics2D g2= (Graphics2D) g;
				Stroke current= g2.getStroke();
